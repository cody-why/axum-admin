...
impl ICacheService for RedisCacheService {
    ...
    async fn get_string(&self, k: &str) -> Result<String> {
        ...
        let result: String = conn
            .get(k)
            .await
            .map_err(|e| Error::Internal(format!("RedisService get_string({}) fail: {}", k, e)))?;
        Ok(result)
    }
    ...
    async fn set_string(&self, k: &str, v: &str, ex: u64) -> Result<String> {
        ...
        if ex == 0 {
            return conn
                .set(k, v)
                .await
                .map_err(|e| Error::Internal(format!("RedisService set_string fail: {}", e)))
    ...
    async fn ttl(&self, k: &str) -> Result<i64> {
        ...
        conn.ttl(k)
            .await
            .map(|v: i64| v)
            .map_err(|e| Error::Internal(format!("RedisService ttl fail:{}", e)))
    }

    async fn remove(&self, k: &str) -> Result<bool> {
        let mut conn = redis_conn!().unwrap();
        conn.del(k).await.map_err(|e| Error::Internal(format!("RedisService del fail: {}", e)))
